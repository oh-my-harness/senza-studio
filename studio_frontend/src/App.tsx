...
export default function App() {
  ...
  const [homeView, setHomeView] = useState<"senza" | "agent-team">("senza");
  ...
  // 连接由 App 统一持有，不挂在 ChatPanel 上——连接是整个项目视图的东西，
  // 不属于某一个面板。
  ...
  if (!projectId && homeView === "agent-team") {
    return (
      <div className="relative h-full w-full flex flex-col bg-gray-50">
        <button
          onClick={() => setShowSettings(true)}
          title="设置"
          className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-200"
        >
          <svg
  ...
  if (!projectId) {
    return (
      <div className="relative h-full w-full flex flex-col items-center justify-center bg-gray-50 gap-6">
        <button
          onClick={() => setShowSettings(true)}
          title="设置"
          className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-700 rounded-lg hover:bg-gray-200"
